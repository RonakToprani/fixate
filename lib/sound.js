...
// The gaze-drift catch — now with personality. A small pool of springy, cartoonish
// "bwoop/wobble" cues that rotate so the sound doesn't get stale over a long session.
// Pass the same rotating index the copy uses; anything works, it just varies the cue.
const CATCH_CUES = [
  // 0: playful descending "bwoop" (spring-loaded whoops)
  (c, t) => {
    tone(c, { freq: 720, bend: 300, start: t, dur: 0.16, type: "triangle", peak: 0.17 });
    tone(c, { freq: 300, bend: 480, start: t + 0.14, dur: 0.1, type: "sine", peak: 0.1 });
  },
  // 1: two-note "uh-oh" (falling minor third, cheeky)
  (c, t) => {
    tone(c, { freq: 587, start: t, dur: 0.12, type: "triangle", peak: 0.16 });
    tone(c, { freq: 466, start: t + 0.13, dur: 0.16, type: "triangle", peak: 0.16 });
  },
  // 2: rubbery "boing" (up-then-down wobble)
  (c, t) => {
    tone(c, { freq: 380, bend: 620, start: t, dur: 0.09, type: "sine", peak: 0.16 });
    tone(c, { freq: 620, bend: 320, start: t + 0.08, dur: 0.14, type: "sine", peak: 0.16 });
  },
  // 3: quick "blip-bloop" wink
  (c, t) => {
    tone(c, { freq: 880, start: t, dur: 0.06, type: "square", peak: 0.09 });
    tone(c, { freq: 523, bend: 392, start: t + 0.07, dur: 0.16, type: "triangle", peak: 0.15 });
  },
];

export function playGazeCatch(variant = 0) {
  const c = ac();
  const t = c.currentTime;
  CATCH_CUES[Math.abs(variant) % CATCH_CUES.length](c, t);
}
