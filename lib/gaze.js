...
export const TUNING = {
  ...
  // Treat a sustained no-face (face out of frame / fully turned away) as a look-away that
  // fires a catch, gated by the same ENTER_MS hysteresis. Off = ignore no-face frames.
  TREAT_NOFACE_AS_AWAY: true,
};
...
export class GazeTracker {
  ...
  process(video, tsMs) {
    ...
    // No face in frame. A *sustained* absence is itself a strong "looked away" signal
    // (turned to your phone, talked to someone, left your seat), so we feed it into the
    // same hysteresis as a drift — a brief 1–2 frame detection dropout won't reach ENTER_MS,
    // so it stays safe against flicker, but a real look-away gets caught. (Set
    // TREAT_NOFACE_AS_AWAY=false to go back to pure ignore-the-frame behavior.)
    if (!s.face) {
      if (this.tuning.TREAT_NOFACE_AS_AWAY === false) {
        this._resetTransitionTimers();
        return { state: "unknown", reason: "no-face", focused: !this._drifting };
      }
      const justCaught = this._applyHysteresis(true, tsMs);
      // Only report "drifting" once hysteresis has actually flipped; during the ramp it's
      // still "unknown" so a momentary dropout doesn't dent focus %.
      return {
        state: this._drifting ? "drifting" : "unknown",
        reason: "no-face",
